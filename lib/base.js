  ...
  if (!creando) {
    creando = sql()`
      CREATE TABLE IF NOT EXISTS marcas (
        ...
        situada     BOOLEAN          NOT NULL DEFAULT true,
        actualizado TIMESTAMPTZ      NOT NULL DEFAULT now()
      )`
      /* `situada` llegó después: las tablas ya creadas no la tienen y
         CREATE TABLE IF NOT EXISTS no las toca. Las marcas anteriores se
         dan por situadas, que es lo que eran. */
      .then(() => sql()`
        ALTER TABLE marcas
        ADD COLUMN IF NOT EXISTS situada BOOLEAN NOT NULL DEFAULT true`)
